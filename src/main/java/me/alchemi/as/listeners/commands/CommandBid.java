...
public class CommandBid implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (Storm.hasPermission(sender, "as.base") && sender instanceof Player) {
			...
			try {
				if (args.length >= 1 && args[0] != "0") Queue.current_auction.bid(Integer.valueOf(args[0].replace(" ", "")), (Player) sender);
				...
				if (args.length == 2 && args[1] != "0") Queue.current_auction.bid(Integer.valueOf(args[1].replace(" ", "")), (Player) sender, true);
